# Documentation illustrée du modelet Contribuables
...
```
### Note
* ```tbl:taxpayerLabel``` est une super propriété pour ```tbl:taxpayerFullName``` (personne physique) and ```tbl:taxpayerOrganisationName``` (personne morale). Elle peut-être utilisée quand le type du contribuable (physique ou morale) n'est pas explicité 
* ```tbl:taxpayerAddressEntity``` peut être utilisée quand la valeur de ```tbl:taxpayerAddress``` a été liée à une entité géographique existante dans un KG. Il peut par example s'agir d'une instance de la classe ```addr:Landmark```.
